...
with open('downloads/script.json', 'r') as f:
...
string_data = str(jsondata)
upper_case_data = string_data.upper()
removed_pieces_data = upper_case_data.replace('<P>', '').replace('</P>', '').replace('<B>', '').replace('</B>', '').replace('<I>', '').replace('</I>', '')

for i in unnecessary_punctuation:
    removed_pieces_data.replace(i, ' ')
    clean_data = removed_pieces_data.replace(i, ' ')
...
def Collatz(start_number):
    ...
    result = {"Collatz Sequence": collatz_sequence, "Collatz Length": collatz_length, "Max Collatz": max_collatz, "Sequence Sum": sequence_sum}
    print(result)
...
def Collatz_winner(number_list):
    ...
    for i in range(number_list):
        ...
        collatz_length = len(collatz_sequence)
        max_collatz = max(collatz_sequence)
        list_length += collatz_length
        list_max += max_collatz

        if max(list_length)[i] == max(list_max)[i]:
            return i 
        else:
            print('None')
